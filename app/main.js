...
try {
    ...
    // We add ipcRenderer as a property to window so that remote scripts can use it.
    window.ipcRenderer = ipcRenderer;
    // Set by config file 
    let showAdvancedErrors;
    let outputFixedNumber;
    let angleMode;
    ...
    // Save the original mathjs trig functions
    ...
    // Import new trig functions that acknowledge angleMode, based off the original functions
    mathjs.import({
        sin: function (angle) {
    ...
    // This object holds all the properties used in the calculator logic
    const calculator = {
        ...
        set activeOperator(value) {
            ...
            // Show the active operand in the UI whenever it is set
    ...
    keysContainer.addEventListener('click', (event) => {
        ...
        try {
            ...
            // ony happens when you rapidly click the mouse like a madman while moving it over the keys.
            if (target == null) {
                return;
            }
    ...
    // Functionallity for the AC button
    function allClear() {
        calculator.activeOperator = null;
        calculator.firstOperand = [];
    ...
    // Return the current input as a vector (an array of numbers)
    ...
    // This is used to copy vectors from the output field back into the input field
    ...
    // Display which operator we are using to the UI
    ...
    /* Removes the latest outputs up to the latest result output.  This is used when pressing AC, 
     * to remove everything that has not yet been calculated.
    */
    ...
    // Converts operators from their name to their symbol.  For example, 'subtract' -> '-'
    ...
    // If anything breaks, we can catch the errors.
} catch (err) {
